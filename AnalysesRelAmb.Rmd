...
```{r libraries, options}
lapply(
  c("plyr","dplyr","tidyr","ggplot2","readxl","readr","lme4","afex","lmerTest","emmeans","pbkrtest","magrittr"),
  require,
  character.only = T
)
```
...
should use the new emmeans instead of lsmeans I think
...
df_trials %>% filter(Inclusion & primeSource != "noun") %>% {
  fitAfex2 <<- mixed(
    logRT ~ Condition + (1|Target) + (Condition|Subject),
    data=., method = "KR",
    check_contrasts = FALSE
  )
}
...
df_trials %>% filter(Inclusion & primeValence != "Base") %>% {
  fitAfex3 <<- mixed(
    logRT ~ primeType * Condition + (1|Target) + (1|Subject),
    data=., method = "KR",
...
emmeans(fitAfex3,"Condition")
emmeans(fitAfex3,"primeType")

fitAfex3 %>% emmeans(c("primeType","Condition")) %>% contrast(method="pairwise") %>%
  as_tibble %>%
  filter(
    contrast == "nounPos,Congruent - otherPos,Congruent" |
    contrast == "nounPos,Incongruent - otherPos,Incongruent" |
    contrast == "nounNeg,Congruent - otherNeg,Congruent" |
    contrast == "nounNeg,Incongruent - otherNeg,Incongruent"
  )

# emmeans(fitAfex3, "primeType", by="Condition") # Doesn't test for multiple testing enough!!!
```
