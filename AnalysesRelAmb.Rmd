...
lapply(
  c("plyr","dplyr","tidyr","ggplot2","readxl","readr","lme4","lmerTest","pbkrtest","magrittr"),
  require,
  character.only = T
)
```
...
df_trials %>% filter(Inclusion) %>% {
  fitNull <<- lmer(
    logRT ~ 1 + (1|Target) + (Condition|Subject), data=., REML = TRUE
  )
  # fitType <<- lmer(
  #   logRT ~ primeType * Condition + (1|Subject), data=., REML=TRUE
  # )
  fitJudd <<- lmer(
    logRT ~ Condition + (1|Target) + (Condition|Subject), data=., REML=TRUE # ADD primeType
  )
  # fitSource <<- lmer(
...
lapply(
  c(fitNull,fitType,fitJudd,fitSource),
  function(x) ls_means(x,ddf="Kenward-Roger")
)
