...
df_dir %>% spread(otherDir,valDir) %>%
  ggplot(aes(x=Pos,y=Neg,colour=otherValence2,size=Felt)) +
  geom_point(alpha=.4,position=position_dodge2(width=0,reverse=TRUE)) +
  labs(title="Direct Measures of Positivity, Negativity, & Felt Ambivalence",
...
fullMeans %>% as_tibble %>% mutate(rt = emmean %>% exp) %>%
...
#####################################
## Reaction Times & Direct Measures
#####################################
...
contrRTDM %>% filter_at(vars(primeA,primeB),any_vars(equals(.,"otherAmb"))) %>%
  filter(targetA == targetB & primeA != "letterStr") %>%
  group_by(primeA,primeB) %>%
  do(
    plots = ggplot(.,aes(x=primeA,y=estimate,fill=targetA)) +
      geom_col(position = position_dodge2()) +
      labs(x="",y="",title=paste(.$primeA,"-",.$primeB)) +
...
```{r more graphs, options}

fullRTDM %>% as_tibble %>% mutate(rt = emmean %>% exp) %>%
  ggplot(aes(x=primeDirect,y=rt,fill=targetType)) +
  geom_col(position=position_dodge2()) +
  labs(x="Primes",y="Reaction Times (ms)") +
  guides(fill=guide_legend(title="Target\nValence")) +
  scale_fill_manual(values=c('#00BFC4','#F8766D'),labels=c('Positive','Negative')) +
