...
```{r demographics, options}

df %>% filter(Subject %>% duplicated %>% not) %>%
  summarize(
    n = n(),
    n_Female = Gender %>% equals("weiblich") %>% sum,
    perc_Female = n_Female %>% divide_by(n),
    n_Male = Gender %>% equals("männlich") %>% sum,
    perc_Male = n_Male %>% divide_by(n),
    # n_Other = Gender %>% equals("X") %>% sum,
    # perc_Other = n_Other %>% divide_by(n),
    mean_Age = Age %>% mean,
    sd_Age = Age %>% sd,
    n_LangGerman = Language %>% equals("Deutsch") %>% sum,
    perc_LangGerman = n_German %>% divide_by(n),
    # n_LangOther = Language %>% equals("Sonstiges") %>% sum,
    # perc_LangOther = N_LangOther %>% divide_by(n),
    n_HandR = Handedness %>% equals()
  )
...
} %>% mutate("%" = extract2(.,1,"n") %>% divide_by(n,.))
