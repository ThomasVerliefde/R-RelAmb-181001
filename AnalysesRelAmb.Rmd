...
gg_value_between <- function(data, value, sample_col, sample1, sample2, mean_col, group_col = NA,
                             group1 = NA, group2 = NA, dodge.width = 0.9, extra_y_space = 0, ...)
{
    if(!is.na(group_col))
    {
...
df_dir %>% spread(otherDir,valDir) %>%
  {
    ggplot(data=.,aes(x=Pos,y=Neg,shape=otherValence2,colour=Felt)) +
      geom_point(alpha=.6,size=3,position=position_dodge2(width=0,reverse=TRUE)) +
      labs(x="Positivity",y="Negativity") +
      guides(
        shape = guide_legend(title="Acquaintance\nValence",reverse=TRUE,override.aes = list(alpha=1,size=5)),
  ...
  ggdraw() +
  draw_text("Direct Measures of Acquaintances",x=.45,y=.95)
...
df_val %>%
  ...
  mutate_at(
    ...
    car::leveneTest(Amb ~ otherValence2) %>% print
    car::leveneTest(Pos ~ otherValence2) %>% print
    car::leveneTest(Neg ~ otherValence2) %>% print
    car::leveneTest(Griff ~ otherValence2) %>% print
  }
...

{
  ...
  draw_label(ggCritContrasts$p[[3]],x=.46,y=.60,size = 9) +
  ...
  draw_label(ggCritContrasts$p[[4]],x=.46,y=.395) +
  draw_text("Comparisons of Latency between Acquiantances and Univalent Nouns",x=.5,y=.95)
...

{
  ...
  ggdraw() +
  draw_text("Comparisons of Latency between Acquiantances and Letter Strings",x=.5,y=.95)
...

{
  ...
  ggdraw() +
  draw_text("Difference in Latency between Acquiantances and Letter Strings",x=.5,y=.95)
