...
df <- df_raw %>% 
  ...
  mutate(
    ...
    Inclusion = Correct & speedInclusion & subjInclusion & not(nameExclusion),
    logRT = log(RT)
  ) %>%
  ungroup %>%
  arrange(Subject)
...
#   ) %>% distinct %>% arrange(Subject)
# Due to the values being ',' values (and not '.') there were issues with timings in the .csv files
# As such, more work will be necessary to actually determine timings decently

df_trials <- df %>%
  select(
    Subject,Trial,RT,logRT,Prime,primeType,primeSource,primeValence,Target,targetType,Condition,Condition2,
    Answer,Correct,speedInclusion,subjInclusion,nameExclusion,Inclusion,Accuracy,Gender,Key,FirstOthers,Handedness
...
TODO: Revise to work with the df_others instead of df

```{r Names, options}
# 
# test<- df %>% group_by(otherNames,Subject) %>%
...
afex::set_effects_contrasts()
fitAfex2 <- df_trials %>% filter(Inclusion) %>% {
  mixed(
...
fitAfex2
summary(fitAfex2) %$% coefficients %>% zapsmall #Neat function that puts very small numbers to 0

emmeans(fitAfex2,"targetType") 
emmeans(fitAfex2,"primeType")
plannedMeans2 <- emmeans(fitAfex2,~primeType*targetType)

plannedContr2 <- fitAfex2 %>% emmeans(c("primeType","targetType")) %>%
  contrast(method="revpairwise") %>% as_tibble %>%
  separate(contrast,c("contrastA","contrastB"),sep=" - ") %>%
  separate(contrastA,c("cPrimeA","cTargetA"),sep=",") %>%
  separate(contrastB,c("cPrimeB","cTargetB"),sep=",") %>%
...
plannedContr2 %>%
  ...
  # annotate("text",x=0.52,y=0.97,size=6,label="otherNeg") +
  # annotate("text",x=0.52,y=0.47,size=6,label="otherPos")
  



test1 <- plannedContr2 %>% filter(cPrimeA == "otherPos") %>%

ggplot(plannedContr2,aes(x=cPrimeA,y=estimate)) +
  geom_col(aes(fill=cPrimeB,colour=cTargetA),position="dodge")
