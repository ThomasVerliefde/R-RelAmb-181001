...
df <- df_raw %>% 
  ...
  mutate(
    ...
    Inclusion = Correct & speedInclusion & accInclusion & not(nameExclusion),
    logRT = log(RT)
  ) %>%
  ungroup %>%
  arrange(Subject)
...
df_subj <- df %>%
  select(
    Subject,Age,Gender,Language,Handedness,Accuracy,subjExclusion,Study,Key,FirstOthers
