...
df_rawTime <- "Data_RelAmb.zip" %>% {
  ...
  }# %>% unite(col = timeOther,2,3,sep=".")
}
rm(temp)

# A <- df_rawTime %>% filter(X756 %>% is.na) %>% select(-X756) %>% t %>% as_tibble
# A <- df_rawTime %>% filter(X756 %>% is.na) %>% select(-X756) %>% gather(var,val,2:ncol(.)) %>% spread()
# B <- df_rawTime %>% filter(X769 %>% is.na) %>% select(-X769) %>% t %>% as_tibble
# 
# test <- bind_cols(A,B) %>% t

# df_TEST %>% filter(X756 %>% is.na) %>%
#     select(-X756) %>%
#   rename_all(~ c(
#     "timeOther_1",
#     "timeOther_2",
...
df <- df_raw %>% 
  select(
    -c(practicePrimes,practiceTrials,experimentPrimes,experimentTrials)
  ) %>%
  gather(
    "fullTrials",
    "trialsOutput",
