...
df %>% filter(Subject %>% duplicated) %$% sum(fullInclusion %>% not) %>%
...
df %>% filter(Subject %>% duplicated %>% not & fullInclusion) %>% {
  bind_rows(
    count(., "Var" = "Total"),
    count(., "Var" = Gender),
    count(., "Var" = Language),
...
df %>% filter(Subject %>% duplicated %>% not) %>%
  count(Key,FirstOthers,fullInclusion)
...
df %>% group_by(otherNames,Subject) %>%
  summarize %>% tally %>% add_count(n) %>% arrange(nn %>% desc)
