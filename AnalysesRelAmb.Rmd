...
df_val <- df_others %>% filter(subjExclusion %>% not) %>%
  distinct(Subject,otherValence,otherValence2,otherDir,valDir,
           SRIValence,otherSRI,valSRI,catSRI,catSRI_ext,subjExclusion)
...
afex::set_effects_contrasts()
fitDirect2 <- df_val %>% filter(subjExclusion %>% not) %>%
  mixed(
    valDir ~ otherDir * otherValence2 + (1|otherValence) + (1|Subject),
    data=., method = "KR",
    check_contrasts = TRUE,
...
summary(fitDirect2) %$% coefficients %>% zapsmall #Neat function that puts very small numbers to 0

emmeans(fitDirect2,"otherDir") %>% contrast(method="revpairwise")
emmeans(fitDirect2,"otherValence2") %>% contrast(method="revpairwise")
emmeans(fitDirect2,~otherDir*otherValence2) %>% contrast(method="revpairwise")
compDirect2 <- emmeans(fitDirect2,~otherDir*otherValence2) %>% as_tibble
...
df_others %>%
  ...
  distinct(Subject,otherValence,otherValence2,otherNames,otherRel,otherDir,valDir,catSRI,catSRI_ext) %>%
...
These subjects are entirely with issue, as they provide only overwhelmingly positive participants.
As such, they did not follow instructions and will be excluded.
...
plannedContr %>%
  ...
  do(
    plots = ggplot(.,aes(x=cPrimeA,y=estimate,fill=cTargetA)) +
      geom_col(position=position_dodge2(reverse=TRUE)) + ggtitle(paste(.$cPrimeA,"-",.$cPrimeB)) +
      labs(x="",y="") +
      ...
      geom_label(
        aes(
          label=case_when(
            p.value < .001 ~ paste0(sprintf("%.4f",p.value),'***'),
            p.value < .01 ~ paste0(sprintf("%.4f",p.value),'**'),
            p.value < .05 ~ paste0(sprintf("%.4f",p.value),'*'),
            TRUE ~ paste0(sprintf("%.4f",p.value))
          )
...
plannedMeans %>% as_tibble %>%
  ...
  geom_col(position=position_dodge2(reverse=TRUE)) +
  labs(x="Primes",y="Reaction Times in log(ms)") +
  guides(fill=guide_legend(title="Target",reverse=TRUE)) +
  coord_cartesian(ylim=c(6.45,6.70))
