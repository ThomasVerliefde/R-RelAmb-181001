...
df <- df_raw %>% 
  ...
  mutate(
    ...
    primeSource = gsub("^(noun|other|letter)(.)$","\\1",primeType),
...
df_others <- df %>%
  select(
    ...
    valDir,SRIValence,otherSRI,pointTime,valTime,subjInclusion,Study,Key,FirstOthers
  ) %>% distinct %>% arrange(Subject)

df_trials <- df %>%
  select(
    Subject,Trial,RT,logRT,Prime,primeType,primeSource,primeValence,Target,targetType,Answer,Correct,
...
df_others %>% filter(subjInclusion) %>% group_by(pointTime) %>%
  summarize_at(
    "valTime",
    funs(mean,sd,min,max,.args=list(na.rm=T))
  )
...
# This code is only here because I am really proud of it.
# It is no longer run, and is replaced by the chunk above.
# Code doesn't actually work exactly as intended either...
