...
df_val %>% spread(otherDir,valDir) %>%
  ...
  geom_point(alpha=.1,position=position_dodge2(width=0,reverse=TRUE)) +
  labs(title="Grid of Direct Measures") +
...
df_val %>%
  ...
  geom_point(position=position_dodge2(width = .75,reverse=TRUE),alpha=.1) +
...
plannedMeans %>% as_tibble %>% mutate(rt = emmean %>% exp) %>%
  ggplot() +
  geom_col(aes(x=primeType,y=rt,fill=targetType),position=position_dodge2(reverse=TRUE)) +
