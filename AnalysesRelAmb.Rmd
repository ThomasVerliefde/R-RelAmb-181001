...
df_others <- df %>% filter(Block == "experiment") %>%
  select(
    ...
    valDir,SRIValence,otherSRI,valSRI,subjExclusion,Study,Key,FirstOthers
  ) %>% distinct %>% arrange(Subject)

df_val <- df_others %>% filter(subjExclusion %>% not) %>%
  distinct(Subject,otherValence,otherValence2,otherDir,valDir,SRIValence,otherSRI,valSRI)
...
compSRI <- emmeans(fitSRI, ~SRIValence * otherValence2) %>% as_tibble
...
compSRI %>% 
  ...
  geom_col(position=position_dodge2(reverse=TRUE)) +
  guides(fill=guide_legend(reverse=TRUE)) +
  labs(x="SRI Valence",y="Avg SRI [1-6]",fill="SO Valence") +
...
plannedContr %>%
  ...
  plot_grid(
    ...
    get_legend(ggplot(plannedContr,aes(x=cPrimeA,y=estimate,fill=cTargetA))+geom_col()),
    rel_widths = c(3,.5)
  )
```
