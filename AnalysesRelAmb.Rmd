...
lapply(
  c("plyr","dplyr","tidyr","ggplot2","readxl","readr","lme4","lmerTest","magrittr"),
  require,
  character.only = T
)
```
...
df <- df_raw %>% 
  ...
  mutate(
    ...
    primeSource = gsub("^(noun|other|letter)(Pos|Neg|Str)$","\\1",primeType),
...
df_trials <- df %>%
  select(
    Subject,Trial,RT,logRT,Prime,primeType,primeSource,primeValence,Target,targetType,Condition,Answer,Correct,
...
```{r Multilevel, options}

df_trials %>% filter(subjInclusion & Inclusion) %>% {
  fitNull <<- lmer(logRT ~ 1 + (1|Subject), data=., REML = TRUE);
  fitType <<- lmer(logRT ~ primeType * Condition + (1|Subject), data=., REML=TRUE);
  fitSource <<- lmer(logRT ~ primeSource * Condition + (1|Subject), data=., REML=TRUE)
}
