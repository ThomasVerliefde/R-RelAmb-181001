...
df <- df_raw %>% 
  ...
  separate(
    ...
    extra = "merge",
    fill = "right"
  ) %>% # Currently, otherValence is weird, repeating over trials
  mutate(
 ...
 gather(
  ...
  mutate(Griff = ((Pos + Neg) / 2) - abs(Pos - Neg)) %>% # We could add a scaled Griffin Index, but with 0 still 0
  rename(Felt = Amb) %>%
  gather("otherDir","valDir",Pos,Neg,Felt,Griff) %>%
  mutate(
  speedInclusion = (RT >= 300 & RT <= 3000), # Trials with latencies < 300 and > 3000 were excluded
...
# griffinFormula <- function(Pos,Neg){
#   return((Pos+Neg)/2-abs(Pos-Neg))
# }
