...
emmeans(fitAfex2,"targetType") %>% contrast(method="revpairwise")
primeContr2 <- emmeans(fitAfex2,"primeType") %>% contrast(method="revpairwise")
...
fullContr2 <- fitAfex2 %>% emmeans(c("primeType","targetType")) %>%
  contrast(method="revpairwise") %>% as_tibble %>%
  separate(contrast,c("contrastA","contrastB"),sep=" - ") %>%
  separate(contrastA,c("cPrimeA","cTargetA"),sep=",") %>%
  separate(contrastB,c("cPrimeB","cTargetB"),sep=",") %>%
...
primeContr2 <- fitAfex2 %>% emmeans("primeType") %>%
  contrast(method="revpairwise") %>% as_tibble %>%
  separate(contrast,c("contrastA","contrastB"),sep=" - ") %>%
